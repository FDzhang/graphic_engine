...
CAvmTimeStitcherNodeNew::CAvmTimeStitcherNodeNew()
{
...
int CAvmTimeStitcherNodeNew::InitNode(IXrCore* pXrcore)
{
	int meshid;
	int modelId;
	float *pData; 
	...
	m_timeStitchNode = new SVNode2DStichNew;
...
//render stich aera and ground aera.
    ...
    //render ground area
    int groundMtlId = m_stitchViewNode->CreateMaterial(Material_Rigid_Texture, &m_groundMtl);
    m_groundMtl->SetDiffuseMap(m_timeStitchNode->GetGroundTextureId());
    IMesh *pGroundMesh;
    int groundMeshId = m_stitchViewNode->CreateMesh(ModelType_Plane_Dynamic, 2, 2, 0, "Ground", &pGroundMesh);
    INode *pGroundNode;
    int groundMdlId = m_stitchViewNode->CreateModel(0, groundMtlId, -1, InsertFlag_Default, 0, 0, 0, 1.0, &pGroundNode);
...
int CAvmTimeStitcherNodeNew::UpdateNode()
{
	float steer_angle;
...
int CAvmTimeStitcherNodeNew::SetVisibility(unsigned char pVisibilityFlag)
{
...
int CAvmTimeStitcherNodeNew::SetClear(unsigned char pColorFlag, unsigned char pDepthFlag)
{
	m_stitchViewNode->SetClear(pColorFlag, pDepthFlag);
	return TIME_STITCHER_NORMAL;
}
int CAvmTimeStitcherNodeNew::AddOverlay(IAvmOverlay * pOverlay)
{
...
int CAvmTimeStitcherNodeNew::UpdateStich2DReslt()
{
	IMesh *pMeshTemp;
...
int CAvmTimeStitcherNodeNew::UpdateExternCalib2DReslt()
{
	GLfloat *pData;
	int data_size;
	GLushort *pIndex;
