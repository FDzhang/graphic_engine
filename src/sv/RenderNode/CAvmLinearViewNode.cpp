...
//#define SIDEMIRRORDEGREE 0.5
    ...
    m_rear_afVertices[0]= (GLfloat*)malloc(m_meshWidth*m_meshHeight*8*4);
    m_rear_afVertices[1] = (GLfloat*)malloc(m_meshWidth*m_meshHeight*8*4);
    m_rear_afVertices[2] = (GLfloat*)malloc(m_meshWidth*m_meshHeight*8*4);
    ...
    for(int index=0;index<3;index++)
    {
		...
		else if(index == 1)
		{
			...
			x_pos[1]= -PlaneScaleX;
			x_pos[0]= -PlaneScaleX+UIZ1/tan(theta);
            z_pos[1]= UIZ1;
			z_pos[0]= 0;//UIZ1+PlaneScaleY*tan(theta);
		...
		else
		{
        	...
        	Transfom = XRMat4::RotationY(3.1415926*(SIDEMIRRORDEGREE)) ;	
			x_pos[0]= PlaneScaleX;
			x_pos[1]=  PlaneScaleX-UIZ1/tan(theta);
            z_pos[0]= UIZ1;
			z_pos[1]=0;// UIZ1+PlaneScaleY*tan(theta);
        ...
        for (y=0; y<m_meshHeight; y++) {
            for (x=0; x<m_meshWidth; x++) {
                ...
                m_front_afVertices[index][slotId*8+3] = (((float)x)/(m_meshWidth-1)-0.5)*2* PlaneScaleX;
                m_front_afVertices[index][slotId*8+4] = (((float)y)/(m_meshHeight-1)-0.5)*2* PlaneScaleY;
                m_front_afVertices[index][slotId*8+5] = UIZ1;
                m_rear_afVertices[index][slotId*8] = m_front_afVertices[index][slotId*8];
                ...
                m_front_afVertices[index][slotId*8+3] = x_pos[0]+x*(x_pos[1]-x_pos[0])/(m_meshWidth-1);
                m_front_afVertices[index][slotId*8+4] = PlaneScaleY_NEG +(PlaneScaleY-PlaneScaleY_NEG)*y/m_meshHeight;//(((float)y)/(m_meshHeight-1)-0.5)*2* PlaneScaleY;
                m_front_afVertices[index][slotId*8+5] = z_pos[0]+x*(z_pos[1]-z_pos[0])/(m_meshWidth-1);
                m_rear_afVertices[index][slotId*8] = m_front_afVertices[index][slotId*8];
                m_rear_afVertices[index][slotId*8+1] = m_front_afVertices[index][slotId*8+1];
                m_rear_afVertices[index][slotId*8+2] = m_front_afVertices[index][slotId*8+2];
                #endif
                ...
                m_rear_afVertices[index][slotId*8+7] = m_front_afVertices[index][slotId*8+7];
