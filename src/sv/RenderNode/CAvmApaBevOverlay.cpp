...
static char StopSignName[MAX_NAME_LENGTH] = "stop_sign.dds";
static char EndSignName[MAX_NAME_LENGTH] = "end_sign.dds";
static char ArrowFoward[MAX_NAME_LENGTH] = "arror_foward.dds";
static char ArrowBackward[MAX_NAME_LENGTH] = "arror_backward.dds";
static char Scroll[MAX_NAME_LENGTH] = "scroll.dds";
static char Scroll_Backward[MAX_NAME_LENGTH] = "scroll_backward.dds";
static char ParkLot[MAX_NAME_LENGTH] = "park_lot.dds";
static char UnParkLot[MAX_NAME_LENGTH] = "lock_park_lot.dds";
static char ParkLotDetect[MAX_NAME_LENGTH] = "parking_lot_detection.dds";
static char OnlineEnable[MAX_NAME_LENGTH] = "online_calib_enable.dds";
static char OnlineAPAConflict[MAX_NAME_LENGTH] = "online_conflict.dds";
static char UnLockParkLot[MAX_NAME_LENGTH] = "unlock_park_lot.dds";
...
int CAvmApaBevOverlay::Init(ISceneNode* pRootNode)
{
	if(pRootNode == NULL)
	{
		return AVM_OVERLAY_GET_NODE_FALIED;
	}
	m_rootNode = pRootNode;
	
	m_calib_rslt.center_x = AVMData::GetInstance()->m_2D_lut->GetCalibReslt(POS_CALIB_CX);
	m_calib_rslt.center_y = AVMData::GetInstance()->m_2D_lut->GetCalibReslt(POS_CALIB_CY);
	m_calib_rslt.ppmm_x= AVMData::GetInstance()->m_2D_lut->GetCalibReslt(POS_CALIB_PPMMX);
	m_calib_rslt.ppmm_y= AVMData::GetInstance()->m_2D_lut->GetCalibReslt(POS_CALIB_PPMMY);

	InitApaOverlay(m_rootNode, XR_RES, m_calib_rslt);
...
int CAvmApaBevOverlay::InitApaOverlay(ISceneNode *pSeneNode,char *pFilePath,AVM_Calib_2D_T calib_rslt)
{	
...
int CAvmApaBevOverlay::InitRectNode(float *pose,INode **pNode,ISceneNode *pSeneNode,char *pTextureName,MaterialType MtlType,float **pBuffer)
{    
	int tempmaterialid;
    IMaterial *tempcarmtl;
    IMesh *pMeshTemp;
...
int CAvmApaBevOverlay::UpdateApaOverlay(APAOverlayStruct OverlayReslt)
{
	ProcessStopSign(OverlayReslt.stop_sign_flag,OverlayReslt.end_parking_sign_flag,OverlayReslt.parking_lot_detect_flag,OverlayReslt.online_detect_flag);
	ProcessArrowSign(OverlayReslt.drive_dirction_indicat);	
	ProcessScrollSign(OverlayReslt.drive_dirction_indicat,OverlayReslt.scroll_rate);
	ProcessParkLotSign(OverlayReslt.park_lot_flag,OverlayReslt.pParkLotPos,OverlayReslt.parking_lot_type);

	return AVM_OVERLAY_NORMAL;
}
int CAvmApaBevOverlay::ProcessStopSign(unsigned char sopt_flag,unsigned char end_flag,unsigned char park_lot_flag,unsigned char online_detect_flag)
{    
...
int CAvmApaBevOverlay::ProcessArrowSign(int direct_indicate)
{
    IMaterial *pTempMaterial;	
...
int CAvmApaBevOverlay::ProcessScrollSign(int direct_indicate,float scroll_rate)
{
	IMaterial *pTempMaterial;	
	IMesh *pMeshTemp;
	float pOutput[4];
...
int CAvmApaBevOverlay::CalScrollPos(float scroll_rate,float *Inrect,float*pOutputRect,int direct)
{
    float top = Inrect[1];
	float left = Inrect[0];
...
int CAvmApaBevOverlay::Cal_World_2_Image_Coordinate(float *out_Model_Coord,float *in_world_coord)
{	  
...
int CAvmApaBevOverlay::ProcessParkLotSign(int park_lot_flag,float *lot_pos,unsigned char park_lot_type)
{    
	IMaterial *pTempMaterial;	
	IMesh *pMeshTemp;
	float pCoordImage[2];
 ...
 * ----------- ----------    -----------
 *  11/01/17   Jensen Wang   Create the CAvmApaBevOverlay class.
\*===========================================================================*/