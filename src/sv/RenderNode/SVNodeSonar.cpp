...
void SVNodeSonar::SetRadarPLDReslt()
{
    ...
    if(((m_sonar_parking_lot[front_right_side_sonar].parking_lot_type != PARKING_LOT_NOT_SIUTABLE)
		&&m_lot_upload_left_right_flag != UPLOAD_LEFT_LOT&&pRadarPldRslt->sGround_Points[2].x < -3000&&m_track_park_lot_flag==0 )
		||((m_sonar_parking_lot[front_right_side_sonar].parking_lot_type != PARKING_LOT_NOT_SIUTABLE)
		&&m_lot_upload_left_right_flag != UPLOAD_LEFT_LOT&&m_track_park_lot_flag==1) )
    {
	...
	&&m_lot_upload_left_right_flag != UPLOAD_RIGHT_LOT&&pRadarPldRslt->sGround_Points[2].x < -3000&&m_track_park_lot_flag==0) 
	||(m_sonar_parking_lot[front_left_side_sonar].parking_lot_type != PARKING_LOT_NOT_SIUTABLE
	&&m_lot_upload_left_right_flag != UPLOAD_RIGHT_LOT&&m_track_park_lot_flag==1))
	{
    	...
    	pRadarPldRslt->sGround_Points[0].x = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_near_bottom];
    	pRadarPldRslt->sGround_Points[0].y = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_near_bottom+1];	
		
    	pRadarPldRslt->sGround_Points[1].x = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_far_bottom];
    	pRadarPldRslt->sGround_Points[1].y = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_far_bottom+1];
		
    	pRadarPldRslt->sGround_Points[2].x = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_near_top];
    	pRadarPldRslt->sGround_Points[2].y = m_sonar_parking_lot[front_left_side_sonar].lot_point[2*rect_point_near_top+1];	
    			...
    			case JUMP_POINT_TOP_EDGE:
					...
					else
					{
						...
						else
						{
							...
							if (fdata >= FINAL_PARKING_LOT_PARALLEL_LENGTH)
							{
