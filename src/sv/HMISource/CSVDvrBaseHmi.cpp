...
//    Log_Error("----------DVR OPEN:%d",mainMenuData.iconStatus[MAIN_MENU_DVR]);
    ...
    if(0 == Dvr_App_Get_GuiLayOut(&dvrGuiLayout))
    {
        ...
        case GUI_LAYOUT_SETUP:
            ...
            m_dvrRecMenuVisibility = 1;
            m_dvrPbMenuVisibility = 1;
            break;
...
int CSVDvrBaseHmi::RefreshHmi()
{
	...
	if(m_dvrRecordTab)
	{
		...
		m_dvrRecordTab->SetMenuVisibility(m_dvrRecMenuVisibility);
	...
	if(m_dvrPlaybackTab)
	{
	    ...
	    m_dvrPlaybackTab->SetMenuVisibility(m_dvrPbMenuVisibility);
