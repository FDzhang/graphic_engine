...
CSVTrainedParkingHmi::CSVTrainedParkingHmi(IUINode* pUiNode, int pUiNodeId): ISVHmi::ISVHmi(pUiNode, pUiNodeId)
{
    ...
    m_tpPointCvtMode = TP_POINT_COORD_CVT_GEELY;
...
int CSVTrainedParkingHmi::Init(int window_width, int window_height)
{
    ...
    tpIconPos[TP_CAPTURE_IMAGE_ICON][1] = window_height - black_width - tpIconHeight[TP_CAPTURE_IMAGE_ICON];
    ...
    tpIconPos[TP_NAVIGATING_ICON][1] = tpIconPos[TP_CAPTURE_IMAGE_ICON][1];
    ...
    if(m_vehicleType == VEHICLE_TYPE_NORMAL)
    {
    ...
    {
        tpIconWidth[TP_DRIVING_CAR] = 31.2;
        tpIconHeight[TP_DRIVING_CAR] = 62.4;  
    }
...
int CSVTrainedParkingHmi::UpdateTpElem(Hmi_Message_T* hmiMsg)
{
    ...
    {
        if(m_vehicleType == VEHICLE_TYPE_GEELY)
        {
            m_tpPointCvtMode = TP_POINT_COORD_CVT_GEELY;
        }
        ...
        if(m_tpControlIconShowNum == 1)
        {
            ...
            if(tpReslt.tp_button.tp_buttonD_flag == TRAINED_PARKING_FINISH_SEARCHING_PATH)
            {
                ...
                m_rotateShowFlag = 1;
                AdapteTpPointCoord(loc_x, loc_y, m_tpPointCvtMode); 
...
int CSVTrainedParkingHmi::RefreshHmiIcon()
{
    ...
    if(m_tpDrivingCarVisibility == 1)
    {
        ...
        if(m_vehicleType == VEHICLE_TYPE_GEELY)
        {
            tpIcon[TP_DRIVING_CAR]->SetRotateZ(-1.0 * RadianToAngle(m_rotateAlpha), m_anchorPointX, m_anchorPointY,m_rotateShowFlag);
        }
        else
        {
...
int CSVTrainedParkingHmi::SetElementsVisibility(unsigned char pFlag)
{
    ...
    m_visibilityStatus = pFlag;
    m_uiNode->SetEnable(m_visibilityStatus);

    if(pFlag == 0)
    {
        m_polygonBox->SetVisibility(pFlag);
		m_hmiScroll->SetVisibility(pFlag);
    }
...
int CSVTrainedParkingHmi::AdapteTpPointCoord(float &pX, float &pY, TpPointCoordCvtModeT pCvtMode) //Unit: m
{
    if(pCvtMode == TP_POINT_COORD_CVT_NORMAL)   
    {
    ...
    else if(pCvtMode == TP_POINT_COORD_CVT_GEELY)
	{
		pX = map_x_offside + map_img_width/50 * pX;
		pY = map_y_offside + map_img_height/50 * pY;
		//Log_Message("pX_1: %0.5f,pY_1 : %0.5f ",pX,pY);
		pX = pX * m_showRegionWidth / map_img_width;
		pY = pY * m_showRegionHeight / map_img_height;

		float tmp = 0.0;
		tmp = map_img_height - map_img_height * pX/m_showRegionWidth;
		pX = map_img_width * pY/m_showRegionHeight;
		pY = tmp;
		//Log_Message("pX_2: %0.5f,pY_2 : %0.5f ",pX,pY);
	}  
