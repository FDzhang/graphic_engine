...
CSVDvrFileListTab::~CSVDvrFileListTab()
{
	...
	{	
		...
		if(i == DVR_FILELIST_TAB_EDIT_ICON
			|| i == DVR_FILELIST_TAB_EDIT_CANCEL
			|| i == DVR_FILELIST_TAB_EDIT_SAVE
			|| i == DVR_FILELIST_TAB_EDIT_DELETE
			|| i == DVR_FILELIST_TAB_EDIT_SELECTED_ALL
			|| i == DVR_FILELIST_TAB_SCROLL_UP
			|| i == DVR_FILELIST_TAB_SCROLL_DOWN)
		{
			SAFE_DELETE(m_baseButtonData[i].icon_file_name[1]);
		}
            
		if(i == DVR_FILELIST_TAB_EDIT_SAVE)
		{
    ...
    //SAFE_DELETE(m_pageNumData.fontTypeMtlName);
    
    SAFE_DELETE(m_noFileTextData.textContent[0]);
    //SAFE_DELETE(m_noFileTextData.fontTypeMtlName);
