										...
										CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
			...
			else
			{
				...
				if(bAllowDecompress)
				{
					...
					if (!pDecompressedData)
					{
						PVRTErrorOutputDebug("PVRTTextureLoadFromPointer error: Unable to allocate memory to decompress texture.\n");
                                              printf("unable to allocate memory to decompress data\n");
						return PVR_FAIL;
					}
				...
				else
				{
					PVRTErrorOutputDebug("PVRTTextureLoadFromPointer error: PVRTC not supported.\n");
					 printf("PVRTC not supported\n");
					return PVR_FAIL;
				}
			}
		}
#ifndef TARGET_OS_IPHONE //TODO
		else if (eTextureInternalFormat==GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG || eTextureInternalFormat==GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG)
		{
			//Check for PVRTCI support.
			...
			{				
				PVRTErrorOutputDebug("PVRTTextureLoadFromPointer error: PVRTC not supported.\n");
				printf("PVRTC not supported step 2222\n");
				return PVR_FAIL;
			}
		}
		else if (eTextureInternalFormat==GL_ETC1_RGB8_OES)
		{
			...
			else
			{
				if(bAllowDecompress)
				{
					...
					{
						PVRTErrorOutputDebug("PVRTTextureLoadFromPointer error: Unable to allocate memory to decompress texture.\n");
						printf(" Unable to allocate memory to decompress texture.after converting to RGBA 8888\n");
						return PVR_FAIL;
					}
				...
				else
				{
					...
					printf("ETC not Suportted step3\n");
					return PVR_FAIL;
				}
			}
		}
...
#endif
			printf("ETC not Suportted step4\n");
	...
	if (eTextureInternalFormat==0)
	{
		...
		printf("ETC not Suportted step5\n");
		return PVR_FAIL;
	}
	...
	if(sTextureHeader.u32NumSurfaces>1)
	{
		...
		printf("ETC not Suportted step6\n");
	...
	if (bIsLegacyPVR)
	{
		...
		for (PVRTuint32 uiFace=0; uiFace<psTempHeader->u32NumFaces; ++uiFace)
		{
			...
			if(glGetError())
			{
				...
				printf("ETC not Suportted step8\n");
				return PVR_FAIL;
			}
		}
	}
	else
	{
		for (PVRTuint32 uiMIPLevel=0; uiMIPLevel<psTempHeader->u32MIPMapCount; ++uiMIPLevel)
		{
			...
			if(glGetError())
			{
				FREE(pDecompressedData);
				PVRTErrorOutputDebug("PVRTTextureLoadFromPointer failed: glTexImage2D() failed.\n");
				printf("ETC not Suportted step9\n");
				return PVR_FAIL;
			}
		}
	}
	...
	{
		PVRTErrorOutputDebug("PVRTTextureLoadFromPointer failed: glTexImage2D() failed.\n");
		printf("ETC not Suportted step10\n");
	...
	if(glGetError())
	{
		...
		printf("ETC not Suportted step11\n");
									...
									CPVRTMap<unsigned int, CPVRTMap<unsigned int, MetaDataBlock> > *pMetaData)
{
	...
	if(bSwapDataEndianness)
	{
		...
		if (ui32VariableSize>1)
		{
	       ...
	       printf("start to call PVRTTextureLoadFromPointer\n ");
