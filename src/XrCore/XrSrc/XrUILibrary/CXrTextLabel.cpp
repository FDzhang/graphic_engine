#include "CXrTextLabel.h"
#include "../Common/XrInternal.h"

CXrTextLable::CXrTextLable(Int32 cornerSize, class IActionTrigger* action, Boolean reserveState):
	m_base(cornerSize),
	m_hit(cornerSize)
{
	m_isPressed = 0;
	...
	CXrTextLable::~CXrTextLable()
{

}

xr_state CXrTextLable::Add(class IUINode* node ,Int32 targetIndex, InsertFlag flag, Int32 x, Int32 y, Int32 w, Int32 h, Float32 opacity, String texBase, String texHit)
{
	Float32 tw,th;
...
xr_state CXrTextLable::AddChinese(class IUINode* node ,Int32 targetIndex, InsertFlag flag, Int32 x, Int32 y, Int32 w, Int32 h, Float32 opacity, String texBase, String texHit)
{
	Float32 tw,th;
	m_rootId = node->CreateSpirit(targetIndex, flag, -1, 1.0, x, y, 0 ,w,h); 
	m_root = node->GetLayer(m_rootId);
	...
	node->CreateChineseTextLayer(m_rootId, InsertFlag_Child, m_textMtl, 0.8, 0, h, &m_text, h*m_textSize);
...
xr_state CXrTextLable::SetTextSize(Float32 size)
{
	Float32 w,h,tw,th;
	w = m_root->GetWidth();
	h = m_root->GetHeight();
...
xr_state CXrTextLable::SetText(String text)
{
	Float32 w,h,tw,th;
	w = m_root->GetWidth();
	h = m_root->GetHeight();
...
xr_state CXrTextLable::SetText(unsigned int text)
{
	Float32 w,h,tw,th;
	w = m_root->GetWidth();
	h = m_root->GetHeight();
...
Void CXrTextLable::SetOpacity(Float32 opacity)
{
	m_opacity = opacity;
	m_base.SetOpacity(opacity);
}

Boolean CXrTextLable::OnTouchEvent(
	/* [in] */Int32 layerId,
	/* [in] */Int32 x,
	/* [in] */Int32 y,
	/* [in] */Int32 type)
