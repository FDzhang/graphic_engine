...
#include "../../3rd/hhdds_internal.h"

#include "GLES2Render.h"

//================================================================================
// helper macros.
static inline void NvSwapChar(UInt8& a, UInt8& b)
{
    UInt8 tmp;
    tmp = a;
    a = b;
...
static inline void NvSwapShort(UInt16& a, UInt16& b)
{
    UInt16 tmp;
    tmp = a;
    a = b;
...
//================================================================================
//================================================================================
#define SIZE_DXTC(WIDTH, HEIGHT, FORMAT) \
        (((WIDTH)+3)/4)*(((HEIGHT)+3)/4)* \
        ((FORMAT) == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT || (FORMAT) == GL_ETC1_RGB8_OES ? 8 : 16)

static Int32 size_image(Int32 width, Int32 height, const NVHHDDSImage* image)
{
    if (image->compressed) 
    {
		return SIZE_DXTC( width, height, image->format);
...
//================================================================================
//================================================================================
static Int32 total_image_data_size(NVHHDDSImage* image)
{
    Int32 i, j, index = 0, size=0, w, h;
    Int32 cubeCount = image->cubemap ? 6 : 1;
...
//================================================================================
//================================================================================
static void flip_blocks_dxtc1(DXTColBlock *line, Int32 numBlocks)
{
...
//================================================================================
//================================================================================
static void flip_blocks_dxtc3(DXTColBlock *line, Int32 numBlocks)
{
    DXTColBlock *curblock = line;
...
//================================================================================
//================================================================================
static void flip_blocks_dxtc5(DXTColBlock *line, Int32 numBlocks)
{
    DXTColBlock *curblock = line;
...
//================================================================================
//================================================================================
static int flip_data_vertical(char *image, Int32 width, Int32 height, 
                               NVHHDDSImage* info)
{
    if (info->compressed) 
    {
        ...
        DXTColBlock *bottom;
        char *tmp;
        void (*flipblocks)(DXTColBlock*, Int32) = NULL;
    ...
    {
        Int32 linesize = width * info->bytesPerPixel;
        Int32 j;
        Int8 *top;
        Int8 *bottom;
        char *tmp;
        ...
        {
            top    = (Int8*)(image + j * linesize);
            bottom = (Int8*)(image + (((height-j)-1) * linesize));
...
//================================================================================
//================================================================================
NVHHDDSImage *NVHHDDSLoad(const char* filename, Int32 flipVertical)
{
    DDS_HEADER ddsh;
    char filecode[4];
    XrFile *fp;
    NVHHDDSImage *image = NULL;
    Int32 i;
    Boolean needsBGRASwap = FALSE;
    Boolean isAllreadyFlipped = FALSE;

    xrprintf(__FUNCTION__, "\r\nNVHHDDSLoad file: %s", filename);

    fp = XrFOpen((String)filename, (String)"rb");
    if (fp == NULL) 
    {
        xrprintf(__FUNCTION__, "Could not open file: %s", filename);
        return NULL;
    }

    // read in file marker, make sure its a DDS file
    XrFRead(filecode, 1, 4, fp);
    if (memcmp(filecode, "DDS ", 4) != 0)
    {
        xrprintf(__FUNCTION__, "!> No DDS marker in file header: %s", filename);
        XrFClose(fp);
        return NULL;
    }
    ...
    if (!image)
    {
        xrprintf(__FUNCTION__, "Failed to allocate DDS image memory block for %s", filename);
        XrFClose(fp);
        return NULL;
    }

    // read in DDS header
    XrFRead(&ddsh, sizeof(ddsh), 1, fp);

    // check if image is a cubempap
    if (ddsh.dwCaps2 & DDS_CUBEMAP)
    {
        const Int32 allFaces = 
        ...
        {
            xrprintf(__FUNCTION__, "Attempt to load cubemap that doesn't contain all faces (unsupported..) from: %s", filename);
            XrFClose(fp);
            NVHHDDSFree(image);
            return NULL;
        }
    ...
    {
        xrprintf(__FUNCTION__, "Attempt to load a volume image (currently not supported..) from: %s", filename);
        XrFClose(fp);
        NVHHDDSFree(image);
        return NULL;
    }
    ...
    if (ddsh.ddspf.dwFlags & DDS_FOURCC) 
    {
        switch(ddsh.ddspf.dwFourCC) 
        {
			...
			case FOURCC_ETC1:
				...
				break;
            default:
                xrprintf(__FUNCTION__, "Unsupported FOURCC code = [0x%x], from file: %s", ddsh.ddspf.dwFourCC, filename);
                XrFClose(fp);
                NVHHDDSFree(image);
                return NULL;
        }
    ...
    else
    {
        ...
        {
            xrprintf(__FUNCTION__, "Image data is not DXTC or supported RGB(A) format: %s", filename);
            XrFClose(fp);
            NVHHDDSFree(image);
            return NULL;
        }
    ...
    flipVertical = (isAllreadyFlipped != (flipVertical ? TRUE : FALSE)) 
        ? 1 : 0;
    ...
    {
        xrprintf(__FUNCTION__, "Failed to allocate memory block for image data storage: %s", filename);
        XrFClose(fp);
        NVHHDDSFree(image);
        return NULL;
    }

    Int32 faces = image->cubemap ? 6 : 1;
    ...
    for (j = 0; j < faces; j++)
    {
        ...
        for (i = 0; i < image->numMipmaps; i++) 
        {
            ...
            XrFRead(image->data[index], image->size[index], 1, fp);
            ...
            if (flipVertical)
            {
                ...
                {
                    xrprintf(__FUNCTION__, "Vertical flip of image data failed: %s", filename);
                    XrFClose(fp);
                    NVHHDDSFree(image);
                    return NULL;
                }
...
//================================================================================
int NVHHDDSSave(const NVHHDDSImage *image, const char* filename, Int32 flagAsFlipped)
{
    DDS_HEADER ddsh;
    char filecode[8] = "DDS ";
    XrFile *fp;
...
void *NVHHDDSAllocData(NVHHDDSImage *image)
{
    ...
    {
        Int32 i;
        Int32 size = total_image_data_size(image);
        image->dataBlock = new UInt8[size]; // no need to calloc, as we fill every bit...
        if (image->dataBlock == NULL)
        {
            xrprintf(__FUNCTION__, "Failed to allocate (%d KB) memory block for image data storage.", size/1024);
            return NULL;
        }

        image->data[0] = image->dataBlock;

        Int32 planes = image->numMipmaps * (image->cubemap ? 6 : 1);

        for (i = 1; i < planes; i++) // account for base plus each mip
        {
            image->data[i] = (void*)(((Int32)(image->data[i - 1]))
                + image->size[i - 1]);
        }
...
void NVHHDDSFree(NVHHDDSImage *image)
{
    ...
    {
        if (image->dataBlock)
            delete[] (UInt8*)(image->dataBlock);
        image->dataBlock = NULL;
        delete image;
        image = NULL;
    }
}
...
static Int8 *s_tempStorage = NULL;
static Int32 s_tempStorageSize = 0;
static Int32 s_srcFormat = 0;
static Int32 s_srcType = 0;
...
//================================================================================
static void SetCurrentFormat(Int32 format, Int32 type)
{
    s_srcFormat = format;
    s_srcType = type;
}
...
//================================================================================
//================================================================================
static void *GetTexelAddress( Int32 texelindex, void *ptex )
{
    void *result;
    ...
    switch (s_srcType)
    {
        case GL_UNSIGNED_BYTE:
            ...
            {
                case GL_RGBA:
                    result = &(((UInt8 *)(ptex))[4*texelindex]);
                    break;
                case GL_RGB:
                    result = &(((UInt8 *)(ptex))[3*texelindex]);
                    break;
                case GL_LUMINANCE_ALPHA:
                    result = &(((UInt8 *)(ptex))[2*texelindex]);
                    break;
                case GL_LUMINANCE:
                    result = &(((UInt8 *)(ptex))[1*texelindex]);
                    break;
                case GL_ALPHA:
                    result = &(((UInt8 *)(ptex))[1*texelindex]);
                    break;
                default:
                    xrprintf(__FUNCTION__, "Unsupported format/type pair [0x%x 0x%x]", (UInt32)s_srcFormat, (UInt32)s_srcType);
                    return NULL;
                    break;
            }
            break;
        ...
        case GL_UNSIGNED_SHORT_5_5_5_1:
            result = &(((UInt16 *)(ptex))[texelindex]);
            break;

        default:
            xrprintf(__FUNCTION__, "Unsupported type [0x%x]", (UInt32)s_srcType);
            return NULL;
    }
...
static Color8888 TexelToColor8888(void *texel)
{
    ...
    UInt8  *ubptr = (UInt8 *) texel;
    UInt16 *usptr = (UInt16 *) texel;
    ...
    switch (s_srcType)
    {
        ...
        case GL_UNSIGNED_SHORT_5_6_5:
            ...
            result.red   = (UInt8) ((((usptr[0] & 0xF800) >> 11) & 0x0000001F) << 3);
            result.green = (UInt8) ((((usptr[0] & 0x07E0) >>  5) & 0x0000003F) << 2);
            result.blue  = (UInt8) ((((usptr[0] & 0x001F) >>  0) & 0x0000001F) << 3);
            result.alpha = 255;
            break;
        case GL_UNSIGNED_SHORT_4_4_4_4:
            ...
            result.red   = (UInt8) ((((usptr[0] & 0xF000) >> 12) & 0x0000000F) << 4);
            result.green = (UInt8) ((((usptr[0] & 0x0F00) >>  8) & 0x0000000F) << 4);
            result.blue  = (UInt8) ((((usptr[0] & 0x00F0) >>  4) & 0x0000000F) << 4);
            result.alpha = (UInt8) ((((usptr[0] & 0x000F) >>  0) & 0x0000000F) << 4);
            break;
        case GL_UNSIGNED_SHORT_5_5_5_1:
            if (s_srcFormat != GL_RGBA)
            {
            ...
            result.red   = (UInt8) ((((usptr[0] & 0xF800) >> 11) & 0x0000001F) << 3);
            result.green = (UInt8) ((((usptr[0] & 0x07C0) >>  6) & 0x0000001F) << 3);
            result.blue  = (UInt8) ((((usptr[0] & 0x003E) >>  1) & 0x0000001F) << 3);
            result.alpha = (UInt8) (255 * (usptr[0] & 0x00000001));
            break;
        default:
            xrprintf(__FUNCTION__, "Unsupported format/type pair [0x%x 0x%x]", (UInt32)s_srcFormat, (UInt32)s_srcType);
            break;
...
//================================================================================
//================================================================================
static void SetTexelColor(Color8888 color, Int32 texelindex, void *ptex)
{
    void *ptr;
...
//================================================================================
//================================================================================
static Color8888 GetTexelColor(Int32 texelindex, void *ptex)
{
    void *ptr;
    ptr = GetTexelAddress(texelindex, ptex);
...
//================================================================================
//================================================================================
static void BoxFilterImage(Int32 w, Int32 h, void *psrc, void *pdst)
{
    Int32 sw, sh, r, g, b, a;
    ...
    for (sh = 0; sh < h; sh+=2)
    {
        for (sw = 0; sw < w; sw+=2)
        {
            ...
            sample.red = (UInt8) (r >> 2);
            sample.green = (UInt8) (g >> 2);
            sample.blue = (UInt8) (b >> 2);
            sample.alpha = (UInt8) (a >> 2);
...
//================================================================================
//================================================================================
NVHHDDSImage *NVCompressToDXT(NVHHDDSImage *srcImage, Int32 toFormat, Int32 bMakeMipmaps)
{
    // VARIABLES -------------------
    ...
    void *p[4];
    UInt8 dxAlpha[4][4]; // caching for alpha values for a given block.
    UInt8 cx, cy;
    UInt8 xmin = 0, ymin = 0, xmax = 0, ymax = 0;
    UInt16 l[4][4];
    UInt16 lmin = 0, lmax = 0;
    UInt16 lcut[3];
    ...
    if (srcImage->cubemap)
    {
        xrprintf(__FUNCTION__, "!> Error: cubemap compression not yet supported.");
        return NULL;
    }
    ...
    if (dstImage->dataBlock == NULL) 
    {
        xrprintf(__FUNCTION__, "!> Error allocating DXT conversion buffer");
        NVHHDDSFree(dstImage);
        return NULL;
    }
    ...
    while (1)
    {
        ...
        xrprintf(__FUNCTION__, "processing mip level %d, %dx%d", uMipLevel, w, h);
        ...
        for (y = 0; y < h; y += 4)
        {
            ...
            for (x = 0; x < w; x += 4)
            {
                ...
                for (cy = 0; cy < 4; ++cy)
                {
                    for (cx = 0; cx < 4; ++cx)
                    {
                        ...
                        else // opaque or DXT3/5
                        {
                            ...
                            l[cy][cx] =    ((UInt16)rgba.red)*3 + ((UInt16)rgba.green)*6 + ((UInt16)rgba.blue);
                            if (bFirst)
                            {
                                bFirst = GL_FALSE;
                                xmin = xmax = cx;
                ...
                else if (toFormat==GL_COMPRESSED_RGBA_S3TC_DXT5_EXT)
                {
                    ...
                    Int32 smallDeltaSpace = 0, smallDeltaVal = 3, smallDeltaTol = 128;
                    ...
                    if (smallDeltaSpace) // use 6pt system.
                    {
                        ...
                        for (i = 2; i < 6; i++)
                        {
                            ...
                            alut[i] = (UInt16) (((6-i)*alut[1] + (i-1)*alut[6]) / 5);
                        }
                        slut[0] = 6;
                        slut[7] = 7;
                        slut[1] = 0;
                    ...
                    else // otherwise, we use a 8-point system, inclusive of our min/max
                    {
                        ...
                        {
                            // should be between 0 and 255...
                            alut[i] = (UInt16) (((7-i)*alut[7] + i*alut[0]) / 7); 
                        }
                        slut[0] = 1;
                        slut[7] = 0;
                        for (i = 1; i < 7; i++)
                        ...
                        for (cx = 0; cx < 4; ++cx)
                        {
                            for (i=0; i<7; i++)
                            {
                                    ...
                                    (dxAlpha[cy][cx] <= alut[i+1]))
                                {
                                    ...
                                    {
                                        dxAlpha[cy][cx] = (UInt8)slut[i];
                                    }
                                    else
                                    {
                                        dxAlpha[cy][cx] = (UInt8)slut[i+1];
                                    }
                ...
                pdxtd->col0 = c[0];
                pdxtd->col1 = c[1];
                pdxtd->row[0] = (UInt8) ((idx>>0)&0xFF);
                pdxtd->row[1] = (UInt8) ((idx>>8)&0xFF);
                pdxtd->row[2] = (UInt8) ((idx>>16)&0xFF);
                pdxtd->row[3] = (UInt8) ((idx>>24)&0xFF);
...
//                    (Int32)((UInt32)pdxtd - (UInt32)(dstImage->data[uMipLevel])) );

        xrprintf(__FUNCTION__, "Finished dxt mipmap %d (finish size %d)", uMipLevel, dstImage->size[uMipLevel]);

        // have we done all the levels yet?
        uMipLevel++;
        if (uMipLevel == (UInt32)dstImage->numMipmaps)
        ...
        if (bMakeMipmaps)
        {
            if (!s_tempStorage || s_tempStorageSize < w*h)
            {
                ...
                s_tempStorage = new Int8[s_tempStorageSize];
        ...
        { // see if we HAVE a next mip level, else we need to bail.
            if ((UInt32)srcImage->numMipmaps < uMipLevel)
                break;

            if (srcImage->data[uMipLevel] == NULL)
                break; // out of the big while loop.
    ...
    xrprintf(__FUNCTION__, "Finished DXT compression.");
