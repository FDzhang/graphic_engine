...
int RGBE_WriteHeader(XrFile *fp, int width, int height, rgbe_header_info *info)
{
...
int RGBE_ReadHeader(XrFile *fp, int *width, int *height, rgbe_header_info *info)
{
  char buf[128];
  int found_format;
  float tempf;
  ...
  if (XrFGets(buf,sizeof(buf)/sizeof(buf[0]),fp) == NULL)
  ...
  if ((buf[0] != '#')||(buf[1] != '?')) {
    ...
    if (XrFGets(buf,sizeof(buf)/sizeof(buf[0]),fp) == 0)
      return rgbe_error(rgbe_read_error,NULL);
  }
...
/* These routines can be made faster by allocating a larger buffer and
   XrFRead-ing and fwrite-ing the data in larger chunks */
int RGBE_WritePixels(XrFile *fp, float *data, int numpixels)
{
...
int RGBE_ReadPixels(XrFile *fp, float *data, int numpixels)
{
  unsigned char rgbe[4];

  while(numpixels-- > 0) {
    if (XrFRead(rgbe, sizeof(rgbe), 1, fp) < 1)
      return rgbe_error(rgbe_read_error,NULL);
...
int RGBE_ReadPixels_Raw(XrFile *fp, unsigned char *data, int numpixels)
{
  if (XrFRead(data, 4, numpixels, fp) < (size_t)numpixels)
    return rgbe_error(rgbe_read_error,NULL);
...
static int RGBE_WriteBytes_RLE(XrFile *fp, unsigned char *data, int numbytes)
{
...
int RGBE_ReadPixels_RLE(XrFile *fp, float *data, int scanline_width,
			int num_scanlines)
{
  ...
  while(num_scanlines > 0) {
    ...
    for(i=0;i<4;i++) {
      ...
      while(ptr < ptr_end) {
	if (XrFRead(buf,sizeof(buf[0])*2,1,fp) < 1) {
	  free(scanline_buffer);
	  return rgbe_error(rgbe_read_error,NULL);
	}
	if (buf[0] > 128) {
	  ...
	  *ptr++ = buf[1];
	  if (--count > 0) {
	    if (XrFRead(ptr,sizeof(*ptr)*count,1,fp) < 1) {
	      free(scanline_buffer);
	      return rgbe_error(rgbe_read_error,NULL);
	    }
	    ptr += count;
...
int RGBE_ReadPixels_Raw_RLE(XrFile *fp, unsigned char *data, int scanline_width,
			int num_scanlines)
{
  unsigned char rgbe[4], *scanline_buffer, *ptr, *ptr_end;
  int i, count;
