...
int bmpread(const char * bmp_file, int flags, XrBuffer * p_bmp_out)
{
   int success = 0;
   ...
   do
   {
      ...
      if(!(ctx.fp = XrFOpen((String)bmp_file, (String)"rb"))) break;
      if(!_bmp_ReadHeader(&ctx))            break;
      if(!_bmp_ReadInfo(&ctx, flags))       break;
      _bmp_InitDecode(&ctx);
      _bmp_Decode(&ctx, flags);

      p_bmp_out->width = (int)ctx.info.width;
      p_bmp_out->height = ctx.lines;
      p_bmp_out->pData = ctx.rgb_data;
	  switch (ctx.info.bits) {
		case 8:
			p_bmp_out->format = XR_R8;
			break;
		case 24:
			p_bmp_out->format = XR_R8G8B8;
			break;
		default:
			p_bmp_out->format = 0;
			break;
   ...
   printf("\r\n Load bmp texture file %s,width = %d, height = %d",bmp_file, p_bmp_out->width,p_bmp_out->height);

   _bmp_FreeContext(&ctx, success);

   return success;
}
int ReadYUVInfo(const char *bmp_file,XrBuffer *p_bmp_out)
{
	int namelen=0;
	char *info_file=0;
	XrFile *pInfo=0;

	namelen = strlen(bmp_file);
	info_file = (char *)malloc(namelen+1);
	memset(info_file,0,namelen+1);
	memcpy(info_file,bmp_file,namelen);
	
	//find config image size config file	
	info_file[namelen-3]='t';
	info_file[namelen-2]='x';
	info_file[namelen-1]='t';
	
	pInfo = XrFOpen((String)info_file,(String)"rb");
	if(!pInfo) return 1;
	fscanf( pInfo , "%d," , &p_bmp_out->width);
	fscanf( pInfo , "%d," , &p_bmp_out->height);
	XrFClose(pInfo);
	free(info_file);
    return 0;
...
//Init image buffer .
static int YUVInitDecode(_bmp_read_context * p_ctx)
{
	// buffer file %s,line %d",__FILE__,__LINE__);
	p_ctx->rgb_data = (uint8_t *)
               malloc(p_ctx->info.width*p_ctx->info.height*3);
	p_ctx->file_data = (uint8_t *)
               malloc(p_ctx->info.width*2);
   return 0;
}
static int YUVDecode(_bmp_read_context * p_ctx, int flags)
{
   ...
   uint8_t *p_v;
   uint8_t *p_temp;
  // XrFRead(p_ctx->rgb_data, p_ctx->info.height*p_ctx->info.width*2, 1, p_ctx->fp);
  int i =0;
   int j=0;

      /* keeping scan lines in order */
      p_rgb      = p_ctx->rgb_data;
      rgb_inc    = p_ctx->info.width*3;
      p_rgb_end  = p_ctx->rgb_data + (p_ctx->info.width*3* (p_ctx->info.height));
	  p_y   = p_ctx->rgb_data;
	  p_u   = p_ctx->rgb_data+(p_ctx->info.width*p_ctx->info.height);
	  p_v   = p_u+p_ctx->info.width/2;
   

   p_line_end = p_rgb + p_ctx->info.width * 3;
   while(p_rgb != p_rgb_end)
   {
	  XrFRead(p_ctx->file_data, p_ctx->info.width*2, 1, p_ctx->fp);
	  for(i=0;i<p_ctx->info.width;i+=2)
	  {
		  p_y[3*i]=p_ctx->file_data[2*i];
		  p_y[3*i+1]=p_ctx->file_data[2*i+1];		  
		  p_y[3*i+2]=p_ctx->file_data[2*i+3];		  
		  p_y[3*i+3]=p_ctx->file_data[2*i+2];
		  p_y[3*i+4]=p_ctx->file_data[2*i+1];		  
		  p_y[3*i+5]=p_ctx->file_data[2*i+3];			  



	  }
      j++;
	  p_y += p_ctx->info.width*3;
	  p_u +=p_ctx->info.width;
	  p_v += p_ctx->info.width;
	  p_rgb += rgb_inc;
	  p_line_end += rgb_inc;
   }
      ...
      /* keeping scan lines in order */
      p_rgb      = p_ctx->rgb_data;
      rgb_inc    = p_ctx->info.width*2;
      p_rgb_end  = p_ctx->rgb_data + (p_ctx->info.width*2* (p_ctx->info.height));
	  p_y   = p_ctx->rgb_data;
	  p_u   = p_ctx->rgb_data+(p_ctx->info.width*p_ctx->info.height);
	  p_v   = p_u+p_ctx->info.width/2;
      ...
      {
         XrFRead(p_ctx->file_data, p_ctx->info.width*2, 1, p_ctx->fp);
		// memcpy(p_y,p_ctx->file_data,p_ctx->info.width*2);
		 
		 for(i=0;i<p_ctx->info.width*2;i++)
		 {
		     if(i%2==1)
		     {
		     p_y[i]=p_ctx->file_data[i];
		     }
			 else
			 {
			     p_y[i]=p_ctx->file_data[i];
		 ...
		 j++;
		 if(j>= p_ctx->info.height/2)
		 	{
		 	//break;
		 	}
		 p_y += p_ctx->info.width*2;
		 p_u +=p_ctx->info.width;
		 p_v += p_ctx->info.width;
         p_rgb += rgb_inc;
        // p_line_end += rgb_inc;
  ...
  {
	 XrFRead(p_ctx->file_data, p_ctx->info.width*2, 1, p_ctx->fp);
	 for(i=0;i<p_ctx->info.width;i++)
	 {
		 p_y[i]=p_ctx->file_data[2*i+1];
		 if(i%2)
		 {
			 p_u[(i-1)/2] = p_ctx->file_data[2*i];
		 }
		 else
		 {
			 p_v[i/2] = p_ctx->file_data[2*i];
		 
		 }
		 
	 }
	 p_y += p_ctx->info.width;
	 p_u +=p_ctx->info.width;
	 p_v += p_ctx->info.width;
	 p_rgb += rgb_inc;
	// p_line_end += rgb_inc;
  ...
  while(p_rgb != p_rgb_end)
  {
	 ...
	 for(i=0;i<p_ctx->info.width;i++)
	 {
		 p_y[i]=p_ctx->file_data[2*i];
		 if(i%2)
		 {
			 p_u[(i-1)/2] = p_ctx->file_data[2*i+1];
		 }
		 else
		 {
			 p_v[i/2] = p_ctx->file_data[2*i+1];
...
int YUVread(const char * bmp_file, int flags, XrBuffer * p_bmp_out)
{
   int success = 0;
   char temp='a';
   ...
   do
   {
      ...
      if(!p_bmp_out) break;
      memset(p_bmp_out, 0, sizeof(bmpread_t));
      if(ReadYUVInfo(bmp_file,p_bmp_out))  break;	  
      if(!(ctx.fp = XrFOpen((String)bmp_file, (String)"rb"))) break;
	  ctx.info.width = p_bmp_out->width;
	  ctx.info.height = p_bmp_out->height;
      YUVInitDecode(&ctx);	 
	  YUVDecode(&ctx, flags);
...
static int _bmp_ReadHeader(_bmp_read_context * p_ctx)
{
   ...
   do
   {
      if(XrFGetc(p_ctx->fp) != 0x42 /* 'B' */)                            break;
      if(XrFGetc(p_ctx->fp) != 0x4d /* 'M' */)                            break;
      XrFRead(&p_ctx->header, sizeof(_bmp_header), 1, p_ctx->fp);
...
static int _bmp_ReadInfo(_bmp_read_context * p_ctx, int flags)
{
   ...
   do
   {
      XrFRead(&p_ctx->info, sizeof(_bmp_info), 1, p_ctx->fp);
      ...
      if(p_ctx->info.bits <= 8)
      {
         ...
         if(XrFRead(p_ctx->palette, sizeof(_bmp_palette_entry),
...
static int _bmp_InitDecode(_bmp_read_context * p_ctx)
{
   return (   (p_ctx->file_data = (uint8_t *)malloc(p_ctx->file_line_len))
           ...
           && !XrFSeek(p_ctx->fp, p_ctx->header.data_offset, SEEK_SET));
}
...
static int _bmp_Decode(_bmp_read_context * p_ctx, int flags)
{
   ...
   if(decoder)
   {
      while(p_rgb != p_rgb_end)
      {
		 XrFRead(p_ctx->file_data, p_ctx->file_line_len, 1, p_ctx->fp);
